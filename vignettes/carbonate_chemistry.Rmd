...
calculate_open_system_unbalanced_ions(pH = 6.8, pCO2 = qty(0.4, "mbar"))
calculate_open_system_unbalanced_ions(pH = 6.8, pCO2 = qty(50, "mbar"))
calculate_open_system_unbalanced_ions(pH = 6.8, pCO2 = qty(200, "mbar"))

# to calculate pH with addition of a specific amount of base
calculate_open_system_pH(pCO2 = qty(0.4, "mbar"), unbalanced_ions = qty(5, "mM"))
calculate_open_system_pH(pCO2 = qty(50, "mbar"), unbalanced_ions = qty(5, "mM"))
calculate_open_system_pH(pCO2 = qty(200, "mbar"), unbalanced_ions = qty(5, "mM"))
```
...
df_base_vs_pH <-
  ...
  mutate(
    ions = calculate_open_system_unbalanced_ions(pH, pCO2, temp = temperature),
...
df <- data_frame(
  buffer = buffer,
  add = calculate_open_system_unbalanced_ions(
    pH = 6.8,
    pCO2 = qty(50, "mbar"),
    buffer = buffer,
    pKa = 7.5
...
df_base_w_buffer_vs_pH <-
  ...
  mutate(
    ions = calculate_open_system_unbalanced_ions(pH, pCO2, buffer = buffer, pKa = pKa),
    `Na+ required [mM]` = ifelse(ions > 0, get_qty_value(ions, "mM"), 0),
    `Cl- required [mM]` = ifelse(ions < 0, -get_qty_value(ions, "mM"), 0),
...
plot_df2 <- 
  crossing(pCO2s, buffers, pH = seq(3, 7.5, by = 0.05)) %>% 
  mutate(
    ions = calculate_open_system_unbalanced_ions(pH, pCO2, buffer = buffer, pKa = pKa) - buffer,
    `Na+ required [mM]` = ifelse(ions > 0, get_qty_value(ions, "mM"), 0),
    `Cl- required [mM]` = ifelse(ions < 0, -get_qty_value(ions, "mM"), 0),
    `DIC [mM]` = calculate_DIC(pH, pCO2) %>% get_qty_value("mM")
  ) %>% 
...
df_pH_switch <- 
  ...
  mutate(
    `new pH` = calculate_open_system_pH(
      pCO2 = pCO2, buffer = buffer, pKa = pKa, unbalanced_ions = ions
    ),
...
# Env. system (inorganic carbon controlled)

For carbonate controlled environmenal systems with known $[DIC]$, the above equations can be combined to substitute the concentration terms in the charge balance equation with the $[DIC]$ and acid dissociation constants. 
...
which yields the overall charge balance equation in terms of pH ($[H^+]$) and dissovled inorganic carbon ($[DIC]$):

$$
[H^+] - \frac{[DIC]\cdot K_1 [H^+]}{[H^+]^2 + K_1 [H^+] + K_1 K_2} - 2 \frac{[DIC]\cdot K_1 K_2}{[H^+]^2 + K_1 [H^+] + K_1 K_2} - \frac{K_w}{[H^+]} = 0 
$$
