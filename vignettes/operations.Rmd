...
To retrieven numeric values as text that includes their units, simply use `get_qty_text()` instead of `get_qty_value()`. This also works well using a lists of quantities to generate e.g. axis tick mark labels with the ideal unit.
...
# use list e.g. for axis tick mark labels
10^(-9:0) %>% map(qty, "M") %>% map_chr(get_qty_text)
```
...
x %>% get_qty_units_with_label()
x %>% get_qty_units_with_label("same label")
x %>% get_qty_units_with_label(c("apple", "banana", "carrot"))
```
