...
test_that("Testing that units work and can be metric scaled", {
  ...
  expect_is(molarity(1, "mM"), "Molarity")
  ...
  expect_error(cht_scale_metric(molarity(1, "mM"), "x"), "not a known metric prefix")
  expect_error({a <- molarity(1, "mM"); a@unit <- "J"; cht_scale_metric(a, "m")}, "not a valid unit")
  expect_equal(cht_scale_metric(molarity(1, "mM"), "n")@unit, "nM")
  expect_equal(cht_scale_metric(molarity(1, "mM"), "n")@.Data, 1e6)
  expect_equal(cht_scale_metric(molarity(1, "M"), "m")@.Data, 1e3)
  expect_equal(cht_scale_metric(molarity(1, "µM"), "")@.Data, 1e-6)
  expect_equal(cht_base_metric(molarity(1, "kM"))@.Data, 1e3)
  expect_equal(cht_best_metric(molarity(0.2, "M"))@unit, "mM")
  expect_equal(cht_best_metric(molarity(-5000, "nM"))@unit, "µM")
  expect_equal(cht_best_metric(molarity(c(100, 1200, 1500), "pM"))@unit, "nM")
  
  # density
  expect_error(density(1, "J"), "not a known concentration")
  expect_is(density(1, "g/L"), "Density")
  ...
  expect_equal(pressure(1, "kPa", scale_to_best_metric = FALSE)@.Data, cht_get_constant("bar_per_pa"))
