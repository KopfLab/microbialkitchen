...
test_that("Testing that ideal gas calculations are working", {
  ...
  # ideal gas molarity
  expect_error(calculate_ideal_gas_molarity_concentration(), "pressure is missing")
  expect_error(calculate_ideal_gas_molarity_concentration(1), "not a pressure")
  expect_error(calculate_ideal_gas_molarity_concentration(qty(1, "mbar")), "temperature is missing")
  expect_error(calculate_ideal_gas_molarity_concentration(qty(1, "mbar"), 1), "not a temperature")
  expect_equal(
    calculate_ideal_gas_molarity_concentration(qty(100, "mbar"), qty(0, "C")) %>% base_metric() %>% as.numeric(),
  ...
  expect_true(is_gas_gas_solubility(s <- calculate_gas_solubility("CO2", qty(25, "C"))))
