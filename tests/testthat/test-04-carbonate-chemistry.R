...
test_that("Testing that carbonate chemistry calculations are working", {
  ...
  expect_error(calculate_open_system_unbalanced_ions(), "pH is required")
  expect_error(calculate_open_system_unbalanced_ions(7), "either pCO2 or H2CO3*")
  expect_error(calculate_open_system_unbalanced_ions(7, pCO2 = 1), "not a pressure")
  expect_error(calculate_open_system_unbalanced_ions(7, `H2CO3*` = 5), "not a molarity")
  expect_error(calculate_open_system_unbalanced_ions(7, pCO2 = qty(100, "mbar"), temperature = 1), "not a temperature")
  expect_error(calculate_open_system_unbalanced_ions(7, pCO2 = qty(100, "mbar"), solubility = 1), "solubility \\* pCO2.* not a molarity")
  expect_error(calculate_open_system_unbalanced_ions(7, pCO2 = qty(100, "mbar"), buffer = 1), "not a molarity")
  expect_error(calculate_open_system_unbalanced_ions(7, pCO2 = qty(100, "mbar"), buffer = qty(1, "mM")), "must provide a pKa")
  expect_warning(calculate_open_system_unbalanced_ions(7, pCO2 = qty(100, "mbar"), `H2CO3*` = qty(1, "mM")), "pCO2 .* ignored")
  ...
  expect_error(calculate_open_system_pH(pCO2 = qty(100, "mbar"), unbalanced_ions = 1), "not a molarity")
  ...
  expect_equal(
    calculate_open_system_pH(qty(c(0.4, 10, 100, 500), "mbar"), unbalanced_ions = qty(1, "mM")) %>% 
      calculate_open_system_unbalanced_ions(qty(c(0.4, 10, 100, 500), "mbar")) %>% 
  ...
  expect_error(calculate_closed_system_unbalanced_ions(), "pH is required")
  expect_error(calculate_closed_system_unbalanced_ions(7), "amount is missing")
  expect_error(calculate_closed_system_unbalanced_ions(7, TIC = 1), "not a amount")
  expect_error(calculate_closed_system_unbalanced_ions(7, TIC = qty(100, "nmol")), "volume is missing")
  expect_error(calculate_closed_system_unbalanced_ions(7, TIC = qty(100, "nmol"), V_gas = 1), "not a volume")
  expect_error(calculate_closed_system_unbalanced_ions(7, TIC = qty(100, "nmol"), V_gas = qty(1, "mL")), "volume is missing")
  expect_error(calculate_closed_system_unbalanced_ions(7, TIC = qty(100, "nmol"), V_gas = qty(1, "mL"), V_liquid = 1), "not a volume")
  expect_error(calculate_closed_system_unbalanced_ions(7, TIC = qty(100, "nmol"), V_gas = qty(1, "mL"), V_liquid = qty(1, "mL"), temperature = 1), "not a temperature")
  expect_error(calculate_closed_system_unbalanced_ions(7, TIC = qty(100, "nmol"), V_gas = qty(1, "mL"), V_liquid = qty(1, "mL"), buffer = 1), "not a molarity")
  expect_error(calculate_closed_system_unbalanced_ions(7, TIC = qty(100, "nmol"), V_gas = qty(1, "mL"), V_liquid = qty(1, "mL"), buffer = qty(1, "mM")), "must provide a pKa")
  ...
  expect_error(calculate_closed_system_pH(TIC = qty(100, "nmol"), V_gas = qty(1, "mL"), V_liquid = qty(1, "mL"), unbalanced_ions = 1), "not a molarity")
  
  # FIXME: test proper result of calculation
  
  # check that closed system unbalanced ions are consistent ========
  expect_equal(
    calculate_closed_system_pH(TIC = qty(c(0.1, 1, 5, 10), "mmol"), qty(10, "mL"), qty(100, "mL"), unbalanced_ions = qty(1, "mM")) %>% 
      calculate_closed_system_unbalanced_ions(TIC = qty(c(0.1, 1, 5, 10), "mmol"), qty(10, "mL"), qty(100, "mL")) %>% 
      get_qty_value("mM") %>% signif(4),
    c(1, 1, 1, 1)
  )
  expect_equal(
