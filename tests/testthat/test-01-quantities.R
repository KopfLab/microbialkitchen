...
test_that("Testing that units work and can be metric scaled", {
  ...
  expect_equal(molarity_concentration(0, "M") %>% get_qty_units(), "M")
  expect_equal(molarity_concentration(0.2, "M") %>% get_qty_units(), "mM")
  expect_equal(molarity_concentration(0.2, "M", scale_to_best_metric = FALSE) %>% get_qty_units(), "M")
  ...
  # empty / infinite / null vectors (no unit scaling)
  expect_equal(qty(NA, "mg") %>% get_qty_units(), "mg")
  expect_equal(qty(NA, "mg") %>% as.numeric(), NA_real_)
  expect_equal(qty(unit = "mg") %>% get_qty_units(), "mg")
  expect_equal(qty(unit = "mg") %>% get_qty_value(), double(0))
  expect_equal(qty(double(0), "mg") %>% get_qty_units(), "mg")
  expect_equal(qty(double(0), unit = "mg") %>% get_qty_value(), double(0))
  expect_equal(qty(Inf, "mg") %>% get_qty_units(), "mg")
  expect_equal(qty(Inf, "mg") %>% as.numeric(), Inf)
  expect_equal(qty(-Inf, "mg") %>% get_qty_units(), "mg")
  expect_equal(qty(-Inf, "mg") %>% as.numeric(), -Inf)
  
  # empty / infinite /null vectors with other number
  expect_equal(qty(c(NA, 0.005), "g") %>% get_qty_value(), c(NA_real_, 5))
  expect_equal(qty(c(NA, 0.005), "g") %>% get_qty_text(), c(NA_character_, "5 mg"))
